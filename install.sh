...
echo -e "${GREEN}====================================================${NC}"
echo -e "${GREEN}     独角数卡 (SIJULY版) 极速部署脚本 v2.1     ${NC}"
echo -e "${GREEN}====================================================${NC}"

# 0. 检查基础工具 wget
if ! command -v wget &> /dev/null; then
    echo -e "${YELLOW}[系统] 检测到未安装 wget，正在安装...${NC}"
    if [ -f /etc/redhat-release ]; then
        yum install -y wget
    elif [ -f /etc/debian_version ]; then
        apt-get update && apt-get install -y wget
    else
        echo -e "${RED}[错误] 无法自动安装 wget，请手动安装后重试。${NC}"
        exit 1
    fi
fi
...
echo -e "配置文件路径: ${INSTALL_DIR}"
echo -e "${YELLOW}提示：请务必在云服务器防火墙放行 8080 端口！${NC}"
